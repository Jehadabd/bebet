...
class DriveService {
  ...
  String _getUserFriendlyMessage(String error) {
    if (error.contains('invalid_token')) {
      return '⚠️ انتهت صلاحية جلسة الاتصال. جاري إعادة المحاولة تلقائياً...';
    } else if (error.contains('انتهت صلاحية التوكن')) {
      return '🔒 انتهت صلاحية تسجيل الدخول. يرجى الذهاب للإعدادات وتسجيل الدخول مجدداً.';
      ...
      return '💾 مساحة Google Drive ممتلئة. يرجى تحرير بعض المساحة.';
    } else if (error.contains('access_denied') || error.contains('forbidden')) {
      return '🚫 تم رفض الوصول. تأكد من أنك تملك صلاحية الوصول لهذا الملف.';
    } else if (error.contains('file_not_found')) {
      return '📂 الملف المطلوب غير موجود أو تم حذفه.';
    } else {
      return '❌ حدث خطأ غير متوقع: ${error.length > 100 ? error.substring(0, 100) + '...' : error}';
    }
  }
