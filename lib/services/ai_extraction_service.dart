...
class AIExtractionService {
  ...
  /// يحاول Groq أولاً، وإذا فشل يستخدم Gemini
  Future<ExtractionResult> extractInvoiceOrReceiptStructured({
    ...
    // المحاولة 1: Hugging Face API (Qwen2-VL - الأولوية)
    if (huggingfaceApiKey.isNotEmpty) {
      print('🟠 المحاولة 1: استخدام Hugging Face API (Qwen2-VL)...');
      try {
    ...
    if (groqApiKey.isNotEmpty) {
      ...
      print('🔄 الانتقال إلى Gemini...\n');
    } else {
      print('⏭️ تخطي Groq (API Key غير متوفر)\n');
    }

    // المحاولة 3: Gemini API (Fallback الأخير)
    if (geminiApiKey.isNotEmpty) {
      print('🟢 المحاولة 3: استخدام Gemini API...');
      try {
