...
void main() async {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // 🛡️ فحص سلامة البيانات المالية عند بدء التطبيق
  // ═══════════════════════════════════════════════════════════════════════════
  // 🔍 فحص سلامة البيانات (للعرض فقط - بدون إصلاح تلقائي)
  // ═══════════════════════════════════════════════════════════════════════════
  try {
    final dbService = DatabaseService();
    
    // الفحص السريع (يستغرق أقل من ثانية)
    print('🔍 جاري فحص سلامة البيانات...');
    final quickCheck = await dbService.performQuickIntegrityCheck();
    
    if (quickCheck.isHealthy) {
      print('✅ البيانات المالية سليمة 100%');
    } else {
      // ⚠️ عرض التحذيرات فقط - بدون إصلاح تلقائي
      print('! تم اكتشاف ${quickCheck.warnings.length} تحذير:');
      for (final warning in quickCheck.warnings) {
        print('   - $warning');
      }
      // ═══════════════════════════════════════════════════════════════════════
      // ❌ تم إيقاف الإصلاح التلقائي - يجب أن يقوم المستخدم بالإصلاح يدوياً
      // إذا أردت الإصلاح، استخدم شاشة "التدقيق المالي" في الإعدادات
      // ═══════════════════════════════════════════════════════════════════════
      print('ℹ️ للإصلاح اليدوي، استخدم شاشة التدقيق المالي في الإعدادات');
    }
  } catch (e) {
    print('⚠️ تعذر فحص البيانات: $e');
